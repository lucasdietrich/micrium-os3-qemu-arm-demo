...
static void fs_file_read()
{
	FS_ERR err;
	FS_FILE *file;
	file = FSFile_Open(fs_path, FS_FILE_ACCESS_MODE_RD, &err);
	LOG_DBG("FSFile_Open file=%p err=%d", file, err);
	...
	fs_rd_match = OS_TRUE;
	for (CPU_CHAR *w = fs_wr_buf, *r = fs_rd_buf; w - fs_wr_buf < fs_wr_len;
	     w++, r++) {
		if (*w != *r) {
	...
	LOG_DBG("FS file %s read matches read = %u", fs_path, (uint32_t)fs_rd_match);
}
...
void app_init(void)
{
	...
	LOG_INF("App_SNTPc_SetClk %s:%u -> %d",
		SNTPc_Cfg.ServerHostnamePtr,
		SNTPc_Cfg.ServerPortNbr,
		sntp);
#endif /* NETWORKING_SNTP_CLIENT */
...
void app_task(void *p_arg)
{
	...
	stm32_mpu_init((void*)0x20000000u);
	...
	for (;;) {
		if (serial_poll_in(serial_console, &c) == 0) {
			...
			{
				uint32_t sum = 0u;
				LOG_INF("sizeof(var)=%u", ARRAY_SIZE(var));
				for (uint32_t i = 0; i < ARRAY_SIZE(var); i++) {
					sum += var[i];
				}
				LOG_INF("var read sum=%u", sum);
				for (uint32_t i = 0; i < ARRAY_SIZE(var); i++) {
					var[i] = i;
					// LOG_INF("w %u", i);
				}
